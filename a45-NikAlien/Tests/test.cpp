...
void testGetList()
{
    ...
    DynamicArray<Dog> copy = *(serv.getDogList());
    assert(copy.getLength() == arr.getLength());
...
void testUserMode()
{
    ...
    DynamicArray<Dog*> list = serv.createListUser("", -1);
    assert(list.getLength() == arr.getLength());
    assert(list.operator[](3)->getName() == arr[3].getName());
    assert(list.operator[](5)->getBreed() == arr[5].getBreed());

    DynamicArray<Dog*> list2 = serv.createListUser("husky", 10);
    assert(list2.getLength() == 2);
    assert(list2.operator[](0)->getName() == "Akira");
    assert(list2.operator[](1)->getName() == "Billy");

    DynamicArray<Dog*> list3 = serv.createListUser("husky", 5);
    assert(list3.getLength() == 1);
